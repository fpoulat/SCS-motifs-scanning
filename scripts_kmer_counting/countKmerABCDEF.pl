...
#   my $num_workers = @ARGV < $opt_p ? @ARGV : $opt_p; # no need to be wasteful :)
 ...
 sub printHits {
   ...
   ##print out the hits
   #my ($dir,$pre,$suf) = ($file =~ /(^.+\/|^)(.+)\.(.+$)/); ## ça merde ici pour extraire le nom de fichier
   #rustine
   my $pre = $file;
   open OUT, ">$pre.counts" || die "Can't create file $pre.counts\n";
   # print OUT join($SEP, $_, $kmer_p->{$_}), "\n" for sort keys %{$kmer_p}; #original
   # modif pour sortie count tab sequence
   print OUT join($SEP, $kmer_p->{$_}, $_), "\n" for sort keys %{$kmer_p};
   close OUT;
